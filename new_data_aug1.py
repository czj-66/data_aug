...
class DataAugmentationOnDetection:
    ...
    def random_rotate(self, img, boxes, rotate_range=(-20, 20), p=0.5):
        """
        随机旋转图像（-20°~20°），保持图片尺寸不变，同步调整边界框坐标
        :param img: PIL图像对象
        :param boxes: 边界框张量 (N,5)，格式 [label, xc, yc, w, h]（归一化后）
        :param rotate_range: 旋转角度范围 (min, max)
        :param p: 执行旋转的概率
        :return: 旋转后的图像、调整后的边界框
        """
        if np.random.random() < p and len(boxes) > 0:
            ...
            # 2. 旋转图像（保持尺寸不变）
            img_rotated = img.rotate(angle, expand=False, resample=Image.BILINEAR)
            
            # 3. 转换边界框格式：归一化(xc,yc,w,h) → 像素级(x1,y1,x2,y2)
            label = boxes[:, 0].reshape(-1, 1)
            xc, yc, w_box, h_box = boxes[:, 1], boxes[:, 2], boxes[:, 3], boxes[:, 4]
            x1 = (w * xc - 0.5 * w * w_box).numpy()
            y1 = (h * yc - 0.5 * h * h_box).numpy()
            x2 = (w * xc + 0.5 * w * w_box).numpy()
            y2 = (h * yc + 0.5 * h * h_box).numpy()
            ...
            for i in range(len(x1)):
                # 顶点坐标：左上(x1,y1)、右上(x2,y1)、右下(x2,y2)、左下(x1,y2)
                points = np.array([
                    [x1[i], y1[i]], [x2[i], y1[i]],
                    [x2[i], y2[i]], [x1[i], y2[i]]
                ])
                ...
                # 执行旋转并还原到原坐标系统
                points_rotated = np.dot(points, rot_matrix.T) + [cx, cy]
                
                # 计算旋转后边界框的最小包围矩形
                x1_rot = np.min(points_rotated[:, 0])
                y1_rot = np.min(points_rotated[:, 1])
                x2_rot = np.max(points_rotated[:, 0])
                y2_rot = np.max(points_rotated[:, 1])
                
                # 裁剪到图像边界内
                x1_rot = np.clip(x1_rot, 0, w)
                y1_rot = np.clip(y1_rot, 0, h)
                x2_rot = np.clip(x2_rot, 0, w)
                y2_rot = np.clip(y2_rot, 0, h)
                
                # 转换回归一化(xc,yc,w,h)格式
                xc_rot = (x1_rot + x2_rot) / (2 * w)
                yc_rot = (y1_rot + y2_rot) / (2 * h)
                w_rot = (x2_rot - x1_rot) / w
                h_rot = (y2_rot - y1_rot) / h
                
                # 过滤无效边界框
                if w_rot > 0 and h_rot > 0:
                    new_boxes.append([label[i][0], xc_rot, yc_rot, w_rot, h_rot])
            
            # 6. 重新构造边界框张量
            if len(new_boxes) > 0:
                boxes_rotated = torch.tensor(new_boxes, dtype=torch.float32)
            else:
                boxes_rotated = torch.empty((0, 5), dtype=torch.float32)
    ...
    def random_bright(self, img, u=80, p=0.5):
        """
        随机亮度变换
        """
        if np.random.random() < p:
            alpha = np.random.uniform(0, u) / 255
    ...
    def add_gasuss_noise(self, img, mean=0, std=0.05, p=0.5):
        """
        :param img: 输入图像（tensor格式，值范围0~1.0）
        :param mean: 高斯噪声的均值（默认0，保证噪声无整体明暗偏移）
        :param std: 高斯噪声的标准差（默认0.05，越大噪声越强）
